...
void run_tests();
void initialise_program_test(FILE *test_fp);
...
// void error_test(FILE *test_fpfp);
...
// void assign_var_test(FILE *test_fp);
// void interpret_varnum_test(FILE *test_fp);
...
// void Set_test(FILE *test_fp);
// void Set_draw_test(FILE *test_fp);

// void If_test(FILE *test_fp);
// void set_if_statement_test(FILE *test_fp);
// void evaluate_if_statement(FILE *test_fp);
// void evaluate_comparator(FILE *test_fp);
// void void evaluate_comparison(FILE *test_fp);

// void Lt_test(FILE *test_fp);
// void Rt_test(FILE *test_fp);
// void Fd_test(FILE *test_fp);
...
void set_if_statement(Program *p, Var_array v);
void evaluate_if_statement(Program *p, Var_array v, Co_ordinates *c, window *w);
Comparator interpret_comparator(Program *p, Var_array v);
int evaluate_comparison(Program *p, Var_array v);
...
double Polish(Program *p, Stack *s, Var_array v);
int operator_found(Program *p, Var_array v);
void check_sufficient_operands(Program *p, Var_array v);
void push_result_to_stack(Program *p, Stack *s, Var_array v);
void push_varnum_to_stack(Program *p, Stack *s, Var_array v);
void Op(Program *p);
...
void run_tests(){
    ...
    initialise_program_test(test_fp);
...
void initialise_program_test(FILE *test_fp){
    ...
    initialise_program(&p, dummy_file);
    //True tests
    if(string_same(p.file_name, dummy_file)){
        if(p.current_word == 0){
...
void set_if_statement(Program *p, Var_array v){
    ...
    p->current_word++;
    p->comp = interpret_comparator(p, v);
    p->current_word++;
    Varnum(p, v);
...
double Polish(Program *p, Stack *s, Var_array v){
    ...
    if(operator_found(p, v)){
        check_sufficient_operands(p, v);
        push_result_to_stack(p, s, v);
    }
    else{
        push_varnum_to_stack(p, s, v);
    }
   
    p->result = Polish(p, s, v);
    return p->result;
}
...
//ensures sufficient operands exist to perform operation on
void check_sufficient_operands(Program *p, Var_array v){
    p->current_word--;
    Varnum(p, v);
    interpret_varnum(p, v);
    p->current_word--;
    Varnum(p, v);
    interpret_varnum(p, v);
    p->current_word += 2;
