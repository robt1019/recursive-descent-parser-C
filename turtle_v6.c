...
void Instruction(Program *p, Var_array v, Co_ordinates *c, window *w){
    ...
    if(string_same(p->words[p->current_word], "SET_DRAW")){
        p->current_word++;
        Set_draw(p, v, c);
        return;
    }
    if(string_same(p->words[p->current_word], "IF")){
        p->current_word++;
        If(p, v, c, w);
        return;
    }
    ...
    if(string_same(p->words[p->current_word], "(")){
        ...
        if(string_same(p->words[p->current_word], ")")){
            ...
            if(evaluate_comparison(p, v, comp, a, b) == 1){
                if(string_same(p->words[p->current_word], "{")){
                    p->current_word++;
                    printf("current word at end of if function = %s\n", p->words[p->current_word]);
                    Instruction_list(p, v, c, w);
                    return;
                }
            }
            else while(!string_same(p->words[p->current_word], "}")){
                p->current_word++;
                if(string_same(p->words[p->current_word], "")){
