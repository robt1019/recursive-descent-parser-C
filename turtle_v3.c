...
void Instruction_list(Program *p, Var_array v, Co_ordinates *c);
void Instruction(Program *p, Var_array v, Co_ordinates *c);
...
void Do(Program *p, Var_array v, Co_ordinates *c);
...
void Fd(double current_angle, double distance, Program *p, Co_ordinates *c);
...
void Main(Program *p, Var_array v){
    ...
    co_ordinates.x_start = 3;
    co_ordinates.y_start = 3;
    if(!string_same (p->words[p->current_word], "{")){
    error("Missing start brace?");
    }
    p->current_word++;
    Instruction_list(p, v, &co_ordinates);
}
...
void Instruction(Program *p, Var_array v, Co_ordinates *c){
    ...
    if(string_same(p->words[p->current_word], "FD")){
        ...
        Fd(c->current_angle, distance, p, c);
        return;
    }    
